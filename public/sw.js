...
function shouldPrefetch(request) {
  ...
  if (connectionSpeed === 'medium' && isLargeResource(url)) {
    return false;
  }
...
function isLargeResource(url) {
  ...
  return largeResourcePatterns.some(pattern => pattern.test(url.pathname + url.search));
}
  ...
  event.waitUntil(
    Promise.all([
      ...
      caches.open(STATIC_CACHE).then((cache) => {
        ...
        return cache.addAll(STATIC_ASSETS.filter(url => !url.startsWith('/_next')));
      }),
...
function recordLoadTime(startTime) {
  ...
  return `
    ...
    <body>
      <div class="offline-content">
        ...
        <p>Don't worry, Trainer! Your Pokemon data is safely cached and you can still browse your collection.</p>
        <button onclick="window.location.reload()">Try Again</button>
        <br><br>
        <a href="/" style="color: rgba(255,255,255,0.8); text-decoration: none;">← Return to DexTrends</a>
      </div>
    </body>
    </html>
  `;
