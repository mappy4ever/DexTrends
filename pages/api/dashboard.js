...
export default async function handler(req, res) {
  ...
  const startDateDb = `${start}-01`; // First day of the start month
  ...
  try {
    ...
    if (errors.length > 0) {
        ...
        const combinedErrorMsg = errors.map(e => e.message).join('; ');
    ...
    const purposeAgg = purposeSpendingData.reduce((acc, item) => {
    ...
    // --- Top Orgs with ID ---
    const orgAgg = orgSpendingData.reduce((acc, item) => {
    ...
    // --- Top Names with ID ---
    const nameAgg = nameSpendingData.reduce((acc, item) => {
    ...
    const heatmapData = heatmapRawData.map(d => [
        d.month_num,
        d.year,
