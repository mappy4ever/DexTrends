  ...
  try {
    ...
    if (level === 'detailed' || level === 'full') {
      try {
        ...
        healthData.circuitBreakers = globalRegistry.getHealth();
        healthData.rateLimiter = getRateLimiterHealth();
      } catch (error) {
        logger.error('Error in detailed health check', { error: error.message });
        healthData.database = { status: 'unhealthy', error: error.message };
      }
    }
    ...
    if (level === 'full') {
      try {
        ...
        healthData.externalAPIs = { status: 'unhealthy', error: error.message };
      }
    }
    ...
    if (healthData.externalAPIs) {
      const apiStatuses = Object.values(healthData.externalAPIs).map(api => api.status);
