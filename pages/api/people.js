...
export default async function handler(req, res) {
  ...
  if (isNaN(personIdNum)) {
    return res.status(400).json({ error: "Invalid personId parameter, must be a number." });
  }

  if (!start || !/^\d{4}-\d{2}$/.test(start) || !end || !/^\d{4}-\d{2}$/.test(end)) {
    return res.status(400).json({ error: "Missing or invalid date parameters: start (YYYY-MM), end (YYYY-MM)" });
  }
  ...
  try {
    ...
    console.error("API Route Exception (People Data):", exception);
