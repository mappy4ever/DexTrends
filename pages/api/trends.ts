...
export default async function handler(
  ...
  const parsedOrgId = (orgIdStr && orgIdStr.toLowerCase() !== 'all') ? parseInt(orgIdStr, 10) : null;

  if (orgIdStr && orgIdStr.toLowerCase() !== 'all' && (parsedOrgId === null || isNaN(parsedOrgId))) {
  ...
  try {
    ...
    const purposeAgg = fetchedTrips.reduce<Record<string, number>>((acc, item) => {
