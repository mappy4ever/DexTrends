...
type Results = Record<string, FetchResult>;

const TestMaps: NextPage = () => {
        ...
        {maps.map(map => (
          <div key={map.name} style={{ border: '1px solid #ccc', padding: '10px' }}>
            ...
            <img 
              ...
              onLoad={() => console.log(`${map.name} loaded successfully`)}
              onError={() => console.error(`${map.name} failed to load`)}
            />
          </div>
        ))}
      </div>
