  ...
  useEffect(() => {
    async function fetchLeaderboard() {
      try {
        ...
        // Fetch full card details for each leaderboard card
        const enrichedCards = await Promise.all(
          data.slice(0, 200).map(async (entry) => {
            try {
              let cardData;
              if (entry.id) {
                cardData = await pokemon.card.find(entry.id);
              } else if (entry.name) {
                const res = await pokemon.card.where({ q: `name:\"${entry.name}\"` });
                cardData = res.data && res.data[0];
              }
              return cardData ? { ...cardData, ...entry } : entry;
            } catch {
              return entry;
            }
          })
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col gap-3 max-w-2xl mx-auto">
        {cards.map((card, idx) => {
          // Fallback for missing or duplicate IDs
          const cardKey = card.id ? `${card.id}` : `${card.name || 'card'}-${idx}`;
          return (
            <div
              key={cardKey}
            ...
            >
              ...
              <Image src={card.images?.small || card.image || '/placeholder.png'} alt={card.name} width={50} height={70} className="rounded mr-3" />
              <div className="flex-1">
                <div className="font-semibold text-lg">{card.name}</div>
                <div className="text-xs text-gray-500">{card.set?.name || card.set || 'Unknown Set'} • {card.rarity || 'N/A'}</div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70" onClick={() => setModalCard(null)}>
          <div className="bg-white rounded-lg p-6 relative max-w-md w-full" onClick={e => e.stopPropagation()}>
            <button
              ...
              aria-label="Close"
            >&times;</button>
            <Image src={modalCard.images?.large || modalCard.image || '/placeholder.png'} alt={modalCard.name} width={200} height={280} className="rounded mb-4 mx-auto" />
            <h2 className="text-xl font-bold mb-2 text-center">{modalCard.name}</h2>
            <div className="text-center text-sm mb-2">Set: {modalCard.set?.name || modalCard.set || 'Unknown Set'}</div>
