      ...
      .then(async (cardData) => {
        ...
        if (cardData.name) {
          try {
            ...
            const filtered = relatedCardsData.data
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <FadeIn>
              ...
              <div className="p-4 rounded-xl shadow-lg" style={{ background: bgGradient }}>
                ...
                <div className="relative cursor-zoom-in mb-4" onClick={() => setMagnifyImage(true)}>
                  <img 
                    ...
                    alt={card.name}
                    className="w-full h-auto rounded-md shadow-sm transition-transform hover:scale-[1.02]"
                  />
        ...
        {(card.attacks?.length > 0 || card.abilities?.length > 0) && (
          <SlideUp delay={100}>
            <div className={`mt-6 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} p-6 rounded-xl shadow-lg`}>
