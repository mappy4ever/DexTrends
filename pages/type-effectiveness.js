...
const EFFECTIVENESS_DESCRIPTIONS = {
...
export default function TypeEffectiveness() {
  ...
  const [selectedDefendType, setSelectedDefendType] = useState(null);
  const [viewMode, setViewMode] = useState('chart'); // 'chart', 'interactive', 'details', 'advanced'
  ...
  const getEffectiveness = (attackType, defendType) => {
  ...
  const EffectivenessResult = ({ attackType, defendType, effectiveness }) => {
    const style = EFFECTIVENESS_DESCRIPTIONS[effectiveness];
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-purple-50 rounded-lg p-4">
            ...
            <div className="space-y-2">
              {['normal', 'dragon', 'psychic'].map(type => (
                <div key={type} className="flex items-center gap-2">
                  <TypeBadge type={type} size="xs" />
        ...
        <div className="max-w-7xl mx-auto py-8">
          ...
          <div className="text-center mb-8">
            ...
            <div className="flex justify-center gap-2 mb-6 flex-wrap">
              {[
                { key: 'chart', label: 'Full Chart', icon: '📊' },
                { key: 'interactive', label: 'Interactive', icon: '🎮' },
                { key: 'details', label: 'Type Details', icon: '📋' },
                { key: 'advanced', label: 'Advanced Tools', icon: '🔬' }
              ].map(mode => (
                <button
                  key={mode.key}
