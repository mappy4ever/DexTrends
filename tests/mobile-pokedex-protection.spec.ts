...
test.describe('Protected Mobile Pokédex Features', () => {
  ...
  test.describe('VirtualPokemonGrid Protection', () => {
    test('should render 2-3 columns based on viewport', async ({ page }) => {
      // Check grid exists
      const grid = await page.locator('[data-testid="pokemon-grid"]');
      await expect(grid).toBeVisible();

      // Check for virtual scrolling container
      const virtualContainer = await page.locator('.virtual-grid-container');
      await expect(virtualContainer).toBeVisible();

      // Verify column count for 390px width (should be 2-3 columns)
      const firstRow = await page.locator('[data-testid="pokemon-card"]').first();
    ...
    test('should have minimum 48px touch targets', async ({ page }) => {
      const card = await page.locator('[data-testid="pokemon-card"]').first();
  ...
  test.describe('BottomSheet Filters Protection', () => {
    test('should open bottom sheet on filter button tap', async ({ page }) => {
      ...
      const bottomSheet = await page.locator('[data-testid="bottom-sheet"], .bottom-sheet');
    ...
    test('should dismiss bottom sheet on backdrop tap', async ({ page }) => {
      ...
      // Tap backdrop
      const backdrop = await page.locator('[data-testid="bottom-sheet-backdrop"], .bottom-sheet-backdrop, .backdrop');
    ...
    test('should preserve filter state after applying', async ({ page }) => {
      ...
      const typeFilter = await page.locator('[data-testid="type-filter"] button, button:has-text("Fire")').first();
      if (await typeFilter.count() > 0) {
        ...
        const filterBadge = await page.locator('.filter-badge, [data-testid="filter-count"]');
  ...
  test.describe('PullToRefresh Protection', () => {
    test('should show pull to refresh indicator', async ({ page }) => {
      ...
      const pullWrapper = await page.locator('[data-testid="pull-to-refresh"], .pull-to-refresh');
      ...
      if (await pullWrapper.count() > 0) {
        ...
        if (box) {
          ...
          const refreshIndicator = await page.locator('.refresh-indicator, [data-testid="refresh-pokeball"]');
  ...
  test.describe('Mobile Search Experience Protection', () => {
    ...
    test('should open search overlay on mobile', async ({ page }) => {
      ...
      const searchOverlay = await page.locator('[data-testid="search-overlay"], .search-overlay, .mobile-search');
  ...
  test.describe('Mobile Layout Protection', () => {
    test('should respect safe areas', async ({ page }) => {
      ...
      const layout = await page.locator('.mobile-layout, [data-testid="mobile-layout"]').first();
    ...
    test('should have mobile-optimized header', async ({ page }) => {
      ...
      const header = await page.locator('header, .header, nav').first();
...
test.describe('Multi-Device Mobile Protection', () => {
  const mobileDevices = [
  ...
  for (const { name, device } of mobileDevices) {
    test.describe(`${name} Protection`, () => {
      test.use({ ...device });

      test('should work on all mobile devices', async ({ page }) => {
        await page.goto('http://localhost:3001/pokedex');
