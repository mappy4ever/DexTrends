...
class SlidingWindowLimiter {
  ...
  async isAllowed(key) {
    ...
    let requests = this.store.get(requestKey) || [];
...
class FixedWindowLimiter {
  ...
  async isAllowed(key) {
    ...
    let count = this.store.get(windowKey) || 0;
...
class DistributedRateLimiter {
  ...
  async isAllowed(key) {
    ...
    let count = this.store.get(distributedKey) || 0;
    count++;
...
export class RateLimiterFactory {
  static create(type = 'sliding', options = {}) {
...
export function getRateLimiterStats(limiter) {
  return {
    ...
    config: {
      ...
      maxRequests: limiter.maxRequests || limiter.baseLimit,
      capacity: limiter.capacity
    },
    store: {
      size: limiter.store?.size() || 0
    },
    timestamp: new Date().toISOString()
  };
}
