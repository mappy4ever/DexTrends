...
class AnalyticsEngine {
   ...
   */
  trackCardView(cardId, cardName, setName, source = 'unknown', userId = null) {
  ...
  trackCardFavorite(cardId, cardName, action = 'add', userId = null) {
  ...
  trackCardPriceCheck(cardId, cardName, priceType, userId = null) {
  ...
  trackSearch(query, resultsCount, filters = {}, userId = null) {
  ...
  trackPokemonView(pokemonId, pokemonName, source = 'unknown', userId = null) {
  ...
  trackDeckAction(action, deckData = {}, userId = null) {
  ...
  trackCollectionAction(action, collectionData = {}, userId = null) {
   ...
   */
  trackUserJourney(step, metadata = {}, userId = null) {
   ...
   */
  updateSessionData(sessionId, eventType, eventData) {
   ...
   */
  async getCardPerformanceAnalytics(cardId, daysBack = 30) {
    try {
   ...
   */
  analyzeCardPerformance(viewData, favoriteData, priceData) {
    const analytics = {
      views: {
   ...
   */
  async getUserBehaviorAnalytics(daysBack = 30) {
    try {
      const sinceDate = new Date(Date.now() - daysBack * 24 * 60 * 60 * 1000).toISOString();
  ...
  analyzeUserBehavior(events) {
    ...
    events.forEach(event => {
      ...
      if (event.event_type === 'card_view' && event.event_data?.card_id) {
        ...
        analytics.popularCards[cardKey] = (analytics.popularCards[cardKey] || 0) + 1;
      }

      // Track search terms
      if (event.event_type === 'search' && event.event_data?.query) {
        analytics.searchTerms[event.event_data.query] = (analytics.searchTerms[event.event_data.query] || 0) + 1;
      }
    });
    ...
    analytics.popularCards = Object.entries(analytics.popularCards)
      .sort(([,a], [,b]) => b - a)
      .slice(0, 20);

    analytics.searchTerms = Object.entries(analytics.searchTerms)
      .sort(([,a], [,b]) => b - a)
      .slice(0, 50);
   ...
   */
  async getSearchAnalytics(daysBack = 30) {
    try {
      const sinceDate = new Date(Date.now() - daysBack * 24 * 60 * 60 * 1000).toISOString();
   ...
   */
  analyzeSearchBehavior(searchEvents) {
    const analytics = {
      overview: {
    ...
    searchEvents.forEach(event => {
      ...
      analytics.queryAnalysis.topQueries[query] = (analytics.queryAnalysis.topQueries[query] || 0) + 1;
    ...
    analytics.queryAnalysis.topQueries = Object.entries(analytics.queryAnalysis.topQueries)
      .sort(([,a], [,b]) => b - a)
      .slice(0, 50);
   ...
   */
  generateSearchInsights(analytics) {
    const insights = [];
    ...
    const maxHour = Object.keys(analytics.trends.byHour).find(
      h => analytics.trends.byHour[h] === Math.max(...hourlyValues)
    );
