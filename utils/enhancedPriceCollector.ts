...
interface CardTrend {
  ...
  setName: string | null;
  variantType: string;
  prices: Array<{ price: number | null; date: string }>;
  currentTrend: string;
  volatility: number;
  priceChange?: number;
...
interface TrendAnalysis {
  ...
  cardTrends: Record<string, CardTrend>;
  generatedAt: string;
}
...
class EnhancedPriceCollector {
   ...
   */
  extractEnhancedPriceData(card: TCGCard): EnhancedPriceEntry[] {
    const priceEntries: EnhancedPriceEntry[] = [];
    const currentTime = new Date().toISOString();
    
    if (card.tcgplayer && card.tcgplayer.prices) {
      const tcgplayerData = card.tcgplayer;
      
      if (!tcgplayerData.prices) return priceEntries;
   ...
   */
  calculatePriceMetrics(prices: PriceData): PriceMetrics {
   ...
   */
  calculateLiquidityScore(prices: PriceData): number {
   ...
   */
  calculateTrendDirection(prices: PriceData): 'bullish' | 'bearish' | 'neutral' {
   ...
   */
  async storeEnhancedPriceData(priceEntries: EnhancedPriceEntry[], jobId: string): Promise<StoreResult> {
   ...
   */
  async generateMarketTrendAnalysis(daysBack: number = 30): Promise<TrendAnalysis | null> {
    try {
      ...
      return this.analyzeTrends(priceData as PriceDataEntry[]);
    } catch (error) {
   ...
   */
  analyzeTrends(priceData: PriceDataEntry[]): TrendAnalysis {
    const cardTrends: Record<string, CardTrend> = {};
    ...
    Object.values(cardTrends).forEach(card => {
      ...
      const trendKey = card.currentTrend as keyof typeof marketOverview.trendDistribution;
      if (trendKey in marketOverview.trendDistribution) {
        marketOverview.trendDistribution[trendKey]++;
      }
    });
   ...
   */
  async scheduleAutomaticCollection(intervalHours: number = 6): Promise<void> {
   ...
   */
  async collectPricesForTrendingCards(limit: number = 200): Promise<CollectionResult> {
    if (this.isRunning) {
    ...
    try {
      ...
      const allPriceEntries: EnhancedPriceEntry[] = [];
  ...
  private async createCollectionJob(jobType: string = 'enhanced_manual'): Promise<CollectionJob | null> {
    try {
      const { data, error } = await supabase
        .from('price_collection_jobs')
        .insert({
  ...
  private async updateCollectionJob(jobId: string, updates: Partial<CollectionJob>): Promise<void> {
    try {
