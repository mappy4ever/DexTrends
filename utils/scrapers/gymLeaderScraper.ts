...
interface ScrapedGymLeaderData {
  [region: string]: GymLeaderData[];
}
...
class GymLeaderScraper {
  private scrapedData: ScrapedGymLeaderData;
  ...
  private async parseGymLeaderData(pageTitle: string, wikitext: string, region: string): Promise<GymLeaderData> {
    const data: GymLeaderData = {
      id: '',
      name: '',
      region: '',
      city: '',
  ...
  // Extract badge name
  private extractBadge(infobox: Infobox, wikitext: string): string {
    // Check infobox
    if (infobox.badge) {
  ...
  // Extract generation
  private extractGeneration(infobox: Infobox, wikitext: string, region: string): number {
  ...
  private async extractTeamData(wikitext: string): Promise<TeamPokemon[]> {
  ...
  private parsePokemonTemplate(templateContent: string): TeamPokemon | null {
    const data: Partial<TeamPokemon> = {};
    ...
    return data.name ? data as TeamPokemon : null;
  }
  ...
  // Load saved data
  async loadSavedData(region: string | null = null): Promise<GymLeaderData[] | ScrapedGymLeaderData | null> {
    try {
      if (region) {
        return await loadDataFromFile<GymLeaderData[]>(`${region}-gym-leaders.json`, 'gym-leaders');
      } else {
