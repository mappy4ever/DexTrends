...
class EnergyScraper {
  ...
  async scrapeEnergyType(energyType) {
    try {
      ...
      // Get page images
      const images = await getPageImages(energyType.page, (title, info) => {
      ...
      console.error(`Error scraping energy type ${energyType.name}:`, error.message);
      return null;
    }
  }
  ...
  async saveAllData() {
    try {
      ...
      console.error('Error saving energy type data:', error.message);
    }
  }
}
