...
class EliteFourScraper {
  ...
  async scrapeEliteFourMember(pageTitle, region, isChampion = false) {
    try {
  ...
  async parseMemberData(pageTitle, wikitext, region, isChampion) {
    const data = {
  ...
  getGenerationFromRegion(region) {
    const regionToGen = {
  ...
  async downloadMemberImages(pageTitle, memberData) {
    try {
  ...
  async saveAllData() {
    try {
      ...
      console.error('Error saving Elite Four data:', error.message);
    }
  }
}
