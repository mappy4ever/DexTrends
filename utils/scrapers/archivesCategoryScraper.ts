...
class ArchivesCategoryScraper {
  private scrapedData: Record<string, CategoryResult>;
  ...
  async fetchCategoryMembers(categoryName: string, continueFrom: string | null = null): Promise<MediaWikiResponse | null> {
  ...
  async getAllCategoryFiles(categoryName: string): Promise<CategoryFile[]> {
    const allFiles: CategoryFile[] = [];
  ...
  private sortByPriority(files: CategoryFile[], categoryKey: string): CategoryFile[] {
    const gameOrder = [
