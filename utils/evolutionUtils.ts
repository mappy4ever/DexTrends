 ...
 */
export const buildEvolutionTree = async (node, currentSpeciesId = null) => {
  ...
  try {
    ...
    try {
      const pokeData = await cachedFetchData(`https://pokeapi.co/api/v2/pokemon/${speciesId}`);
      ...
      logger.error('Error fetching Pokemon data for evolution tree', { error: fetchError.message, speciesId });
    }
    ...
    return {
      ...
      isCurrent: currentSpeciesId && parseInt(currentSpeciesId) === parseInt(speciesId)
    };
  } catch (error) {
    logger.error('Error in buildEvolutionTree', { error: error.message, speciesId: currentSpeciesId });
    return null;
  }
};
 ...
 */
export const getEvolutionChain = async (speciesId) => {
  try {
    const speciesData = await cachedFetchData(`https://pokeapi.co/api/v2/pokemon-species/${speciesId}`);
    if (!speciesData.evolution_chain?.url) {
      return null;
    }
    
    const evolutionData = await cachedFetchData(speciesData.evolution_chain.url);
    ...
    logger.error('Error fetching evolution chain', { error: error.message, evolutionChainUrl });
    return null;
  }
};
 ...
 */
export const hasMultipleForms = (pokemonData) => {
  return pokemonData?.forms && pokemonData.forms.length > 1;
};

/**
 * Get all forms for a Pokemon species
 */
export const getAllForms = async (speciesId) => {
  try {
    const speciesData = await cachedFetchData(`https://pokeapi.co/api/v2/pokemon-species/${speciesId}`);
    ...
    const forms = await Promise.all(
      varieties.map(async (variety) => {
        try {
          const pokemonData = await cachedFetchData(variety.pokemon.url);
          return {
            id: pokemonData.id,
          ...
          logger.error('Error fetching form data', { pokemonName: variety.pokemon.name, error: error.message });
          return null;
        }
      })
    );
    
    return forms.filter(Boolean).sort((a, b) => {
      if (a.isDefault) return -1;
      if (b.isDefault) return 1;
      return a.name.localeCompare(b.name);
    });
  } catch (error) {
    logger.error('Error fetching forms', { error: error.message });
    return [];
  }
};
