...
export async function fetchPokemonWithCache(pokemonId: string | number): Promise<Pokemon> {
  ...
  try {
    ...
    // Cache the result
    await SupabaseCache.setCachedPokemon(Number(pokemonId), pokemonData as unknown as Record<string, unknown>, `pokemon_${pokemonId}`);
...
export async function fetchPokemonSpeciesWithCache(pokemonId: string | number): Promise<PokemonSpecies> {
  ...
  try {
    ...
    // Cache the result
    await SupabaseCache.setCachedPokemon(Number(pokemonId), speciesData as unknown as Record<string, unknown>, `species_${pokemonId}`);
...
export async function fetchPokemonListWithCache(limit = 1000, offset = 0): Promise<PokemonListResponse> {
  ...
  try {
    ...
    await SupabaseCache.setCachedPokemon(listCacheId, listData as unknown as Record<string, unknown>, cacheKey);
