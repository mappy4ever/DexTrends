...
const AppUpdateNotification: React.FC = () => {
  ...
  useEffect(() => {
    if ('serviceWorker' in navigator) {
      ...
      const handleMessage = (event: ServiceWorkerMessage) => {
        if (event.data) {
          switch (event.data.type) {
            case 'UPDATE_AVAILABLE':
              setUpdateAvailable(true);
              setUpdateInfo({
                version: event.data.version || 'New version',
                timestamp: event.data.timestamp || Date.now()
              });
              utils.hapticFeedback('medium');
              logger.debug('App update available', { data: event.data });
              break;
            ...
            case 'APP_UPDATED':
              ...
              setUpdateInfo({
                version: event.data.version || 'Latest version',
                timestamp: event.data.timestamp || Date.now()
              });
              utils.hapticFeedback('heavy');
              logger.debug('App updated successfully', { data: event.data });
    ...
    // Return undefined if serviceWorker is not available
    return;
  }, [utils]);
