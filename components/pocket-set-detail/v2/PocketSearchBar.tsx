...
interface PocketSearchBarProps {
  searchQuery: string;
...
// Convert rarity symbols for display
...
/**
 * PocketSearchBar - Full-featured sticky search and filter bar for Pocket cards
 */
export const PocketSearchBar: React.FC<PocketSearchBarProps> = ({
  searchQuery,
  onSearchChange,
  rarities,
  selectedRarity,
  ...
  const hasActiveFilters = selectedRarity !== '' || selectedSupertype !== '' || selectedType !== '' || searchQuery !== '';
              ...
              <div>
                ...
                <div className="flex gap-1.5 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-hide">
                  ...
                  {types.map((energyType) => (
                    <button
                      key={energyType}
                      onClick={() => onTypeChange(selectedType === energyType ? '' : energyType)}
                      className={cn(
...
export default PocketSearchBar;
