...
type AlertType = 'price_drop' | 'price_rise' | 'percentage_change' | 'trend_reversal';

interface AlertTypeDisplay {
  label: string;
  color: string;
  icon: string;
}
...
export default function PriceAlerts({ userId = null }: PriceAlertsProps) {
  ...
  const deleteAlert = async (alertId: string) => {
    try {
      if (userId) {
        ...
        if (typeof window !== 'undefined' && window.localStorage) {
          const existingAlerts = JSON.parse(localStorage.getItem('dextrends_price_alerts') || '[]');
          const updatedAlerts = existingAlerts.filter((alert: PriceAlert) => alert.id !== alertId);
          localStorage.setItem('dextrends_price_alerts', JSON.stringify(updatedAlerts));
        }
      }
  ...
  const getAlertTypeDisplay = (alertType: AlertType): AlertTypeDisplay => {
