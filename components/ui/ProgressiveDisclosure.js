...
export const Accordion = ({
  ...
  const handleSectionToggle = useCallback((index, isExpanded) => {
...
export const StepByStepDisclosure = ({
  steps,
  currentStep = 0,
  onStepChange,
  allowSkip = false,
...
export const ProgressiveForm = ({
  ...
  const handleSectionComplete = useCallback((sectionIndex, data) => {
        ...
        {sections[currentSection] && (
          <div className="animate-in slide-in-from-right-2 fade-in duration-300">
            {sections[currentSection].content({
              onComplete: (data) => handleSectionComplete(currentSection, data),
              formData
            })}
          </div>
        )}
