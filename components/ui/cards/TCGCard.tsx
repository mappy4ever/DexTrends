...
/**
 * TCGCard - A standardized Trading Card Game component
 * Used for displaying Pokemon TCG cards with full features like swipe, favorites, and prices
 */
const TCGCard = memo(({
  card,
  ...
  const handleFavorite = useCallback((e: React.MouseEvent) => {
    ...
    if (isFavorited) {
      ...
      toast.info(`${normalizedCard.name} removed from favorites`);
    } else {
  ...
  const handleDelete = useCallback((e: React.MouseEvent) => {
    ...
    if (onDelete) {
      onDelete(card);
      toast.info(`${normalizedCard.name} removed`);
    ...
    >
      ...
      <div className="relative w-full card-image-container">
        ...
        */}
        ...
        {onMagnifyClick && (
          <button
            className="absolute top-3 right-3 p-2.5 bg-white/90 dark:bg-stone-800/90 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-all hover:scale-110 hover:bg-white dark:hover:bg-stone-700 magnifier-icon z-10"
            onClick={handleMagnifyClick}
            title="View card details"
            aria-label="View card details"
          >
            <svg className="w-5 h-5 text-stone-700 dark:text-stone-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
// Export both new name and old name for backward compatibility
export { TCGCard, TCGCard as UnifiedCard };
