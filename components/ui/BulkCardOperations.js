...
const BulkCardOperations = ({ 
  ...
  const selectionAnalytics = useMemo(() => {
    ...
    const rarities = selectedCards.reduce((acc, card) => {
      const rarity = card.rarity || 'Unknown';
      acc[rarity] = (acc[rarity] || 0) + 1;
      return acc;
    }, {});

    const sets = selectedCards.reduce((acc, card) => {
      const setName = card.set?.name || 'Unknown';
      acc[setName] = (acc[setName] || 0) + 1;
      return acc;
    }, {});

    const types = selectedCards.reduce((acc, card) => {
  ...
  const bulkActions = {
    ...
    addToCollection: async (collectionName) => {
    ...
    setPriceAlerts: async (threshold, type) => {
