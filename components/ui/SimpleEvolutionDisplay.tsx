...
interface SimpleEvolutionDisplayProps {
  speciesUrl: string;
  currentPokemonId: string;
}

// Simple evolution display that actually works
const SimpleEvolutionDisplay: React.FC<SimpleEvolutionDisplayProps> = ({ speciesUrl, currentPokemonId }) => {
  const [evolutionChain, setEvolutionChain] = useState<Pokemon[] | null>(null);
    ...
    const loadEvolution = async () => {
      ...
      try {
        ...
        const parseChain = async (node: EvolutionNode, evolutionDetails: EvolutionDetails | null = null): Promise<Pokemon[]> => {
          ...
          const evolutions: Pokemon[] = [current];
          
          // Process all evolution branches
          if (node.evolves_to && node.evolves_to.length > 0) {
            for (const evolution of node.evolves_to) {
              const subEvolutions = await parseChain(evolution, evolution.evolution_details?.[0] || null);
              ...
              >
                <div className="relative w-28 h-28">
                  <Image
                    ...
                    onError={(e) => {
                      (e.currentTarget as HTMLImageElement).src = '/dextrendslogo.png';
                    }}
                  />
                </div>
  ...
  let lastDetails: EvolutionDetails | null = null;
...
function formatEvolutionMethod(details: EvolutionDetails): string {
...
export default SimpleEvolutionDisplay;