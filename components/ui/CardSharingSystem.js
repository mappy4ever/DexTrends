 ...
 */
const CardSharingSystem = ({ 
  cards = [], 
  isOpen = false, 
  onClose,
  shareMode = 'single' // 'single' or 'collection'
}) => {
    ...
    const exportData = {
      ...
      cards: cards.map(card => ({
        ...
        ...(exportOptions.includePrices && { 
          price: card.currentPrice || card.price,
  ...
  const exportAsCSV = () => {
    ...
    const rows = cards.map(card => {
      ...
      if (exportOptions.includePrices) row.push(card.currentPrice || card.price || '');
      ...
      if (exportOptions.includeDescription) row.push(card.artist || '');
      return row;
    });
  ...
  const exportAsPDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        if (exportOptions.includeStats) {
          pdf.text(`Set: ${card.set?.name || 'Unknown'}`, 20, yPosition);
          yPosition += lineHeight;
          pdf.text(`Rarity: ${card.rarity || 'Unknown'}`, 20, yPosition);
          yPosition += lineHeight;
        }
        
        if (exportOptions.includePrices && (card.currentPrice || card.price)) {
          pdf.text(`Price: $${(card.currentPrice || card.price)}`, 20, yPosition);
          yPosition += lineHeight;
        }
        ...
        <div className="grid grid-cols-3 gap-6">
          {displayCards.map((card, index) => (
            <div key={card.id} className="text-center">
              ...
              {exportOptions.includePrices && (card.currentPrice || card.price) && (
  ...
  const tabs = [
    { id: 'share', name: 'Share', icon: <ShareIcon className="h-5 w-5" /> },
    { id: 'export', name: 'Export', icon: <DocumentArrowDownIcon className="h-5 w-5" /> }
  ];
