...
const PokemonLoadingScreen = ({ 
  type = "default", // default, facts, silhouette, pokeball
  message = "Loading...",
  showFacts = true,
  duration = 3000 
}) => {
  ...
  useEffect(() => {
    if (type === "silhouette") {
      const interval = setInterval(() => {
        setCurrentSilhouette((prev) => (prev + 1) % silhouettePokemon.length);
      }, 3000);
      return () => clearInterval(interval);
    }
  ...
  const PokeBallSpinner = () => (
    <div className="relative w-20 h-20 mx-auto mb-6">
      {/* Clean Pokéball design with smooth spinning animation only */}
      ...
      {/* CSS for clean pokeball spinning */}
      <style jsx>{`
        .pokeball-spinner {
