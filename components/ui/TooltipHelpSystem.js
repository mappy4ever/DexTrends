...
export const TooltipProvider = ({ children, delay = 500, hideDelay = 0 }) => {
  const [activeTooltip, setActiveTooltip] = useState(null);
  const [tooltipPosition, setTooltipPosition] = useState({ x: 0, y: 0 });
  const timeoutRef = useRef(null);
  const hideTimeoutRef = useRef(null);
  
  const showTooltip = useCallback((content, targetElement, options = {}) => {
...
export const useContextualHelp = () => {
  const [helpData, setHelpData] = useState(new Map());
  
  const registerHelp = useCallback((key, content) => {
    setHelpData(prev => new Map(prev.set(key, content)));
  }, []);
  
  const getHelp = useCallback((key) => {
    return helpData.get(key);
  }, [helpData]);
  
  const removeHelp = useCallback((key) => {
    setHelpData(prev => {
...
export const HelpContentDatabase = {
  pokemon: {
    cards: {
