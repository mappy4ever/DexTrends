...
export const FABProvider = ({ children }) => {
  ...
  const [globalFAB, setGlobalFAB] = useState(null);
  
  const addFAB = useCallback((fabConfig) => {
  ...
  const removeFAB = useCallback((fabId) => {
    setFabs(prev => prev.filter(fab => fab.id !== fabId));
  }, []);
  
  const setGlobalFABConfig = useCallback((config) => {
    setGlobalFAB(config);
  }, []);
...
export const FloatingActionButton = ({
  ...
  }, [disabled, actions.length, isExpanded, onClick]);
...
export const useContextualFAB = () => {
  ...
  const showFAB = useCallback((config) => {
  ...
  const showAddCard = useCallback((onAdd) => {
    showFAB({
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const showShareActions = useCallback((shareOptions = {}) => {
    const shareActions = [
      {
        icon: (
...
export const useScrollFAB = (threshold = 200) => {
