...
const AdvancedSearchInterface = ({
  ...
  useEffect(() => {
    try {
      const history = JSON.parse(localStorage.getItem('pokemon_search_history') || '[]');
  ...
  const executeSearch = useCallback((query = searchValue, saveHistory = true) => {
