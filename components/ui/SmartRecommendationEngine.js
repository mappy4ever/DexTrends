...
const SmartRecommendationEngine = ({ currentCard, userFavorites = [], userCollection = [] }) => {
  ...
  }, [currentCard, userFavorites, userCollection, userBehavior]);
    ...
    for (const card of allCards) {
      ...
      if (currentCard.types && card.types) {
        const typeOverlap = currentCard.types.filter(type => 
          card.types.some(t => t.type ? t.type.name === type.type.name : t === type)
        ).length;
  ...
  const getPersonalizedPicks = async () => {
    ...
    for (const card of allCards) {
      ...
      if (card.types) {
        const typeScore = card.types.reduce((acc, type) => {
          const typeName = type.type ? type.type.name : type;
          return acc + (userInterests.favoriteTypes[typeName] || 0);
        }, 0);
  ...
  const generateSimilarityReason = (cardA, cardB) => {
    const reasons = [];
    
    if (cardA.types && cardB.types) {
      const sharedTypes = cardA.types.filter(type => 
        cardB.types.some(t => (t.type ? t.type.name : t) === (type.type ? type.type.name : type))
      );
      if (sharedTypes.length > 0) {
        reasons.push(`Both are ${sharedTypes.map(t => t.type ? t.type.name : t).join('/')}-type`);
      }
    }
  ...
  const calculateCardPriority = (card, completionRate) => {
    let priority = 0;
    ...
    priority += (rarityScores[card.rarity] || 1) * 10;
  ...
  const handleRecommendationClick = (card, source) => {
...
// Helper components
const RecommendationSection = ({ title, description, cards, onCardClick }) => (
  <div>
    <div className="mb-4">
...
const PriceAlertsSection = ({ alerts, onCardClick }) => (
  <div>
...
const CollectionSuggestionsSection = ({ suggestions, onCardClick }) => (
  <div>
