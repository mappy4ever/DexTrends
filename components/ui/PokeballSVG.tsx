...
interface PokeballSVGProps {
  ...
  className?: string;
  animate?: boolean;
  color?: 'default' | 'shiny' | 'premier' | 'great' | 'ultra' | 'master';
  shiny?: boolean;
  premier?: boolean;
  great?: boolean;
  ultra?: boolean;
...
export const PokeballSVG = ({ size = 40, className = "", animate = false, color = "default" }: PokeballSVGProps) => {
  const colorSchemes = {
    default: {
      top: "#FF0000",
      topGradient: "#FF4444", 
...
export const TypeBadgeSVG = ({ type, size = 24, className = "" }: { type: string; size?: number; className?: string }) => {
  const typeColors = {
    normal: "#A8A77A",
    fire: "#EE8130", 
    water: "#6390F0",
  ...
  const color = (typeColors as any)[type?.toLowerCase()] || typeColors.normal;
...
export const LoadingDotsRun = ({ className = "", color = "#FF0000" }: { className?: string; color?: string }) => (
  <svg 
    width="60" 
    height="20" 
    viewBox="0 0 60 20" 
...
export const PikachuSilhouette = ({ size = 80, className = "" }: { size?: number; className?: string }) => (
  <svg 
    width={size} 
    height={size} 
    viewBox="0 0 100 100" 
...
export const SearchMagnifyingGlass = ({ size = 24, className = "" }: { size?: number; className?: string }) => (
  <svg 
    width={size} 
    height={size} 
    viewBox="0 0 24 24" 
