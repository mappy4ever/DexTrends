 ...
 */
const MarketInsightsDashboard = ({ cards = [], userPortfolio = [] }) => {
  ...
  const marketMetrics = useMemo(() => {
    ...
    const cutoffDate = new Date(now - timeframeMs[timeframe]);

    // Calculate price changes
    const cardsWithChanges = cards.map(card => {
      const currentPrice = parseFloat(card.currentPrice || card.price || 0);
      const historicalPrice = parseFloat(card.previousPrice || currentPrice * 0.95); // Mock previous price
    ...
    // Rarity distribution
    const rarityDistribution = cardsWithChanges.reduce((acc, card) => {
    ...
    // Set performance
    const setPerformance = cardsWithChanges.reduce((acc, card) => {
