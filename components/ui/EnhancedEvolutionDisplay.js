...
function EnhancedEvolutionDisplay({ speciesUrl, currentPokemonId }) {
  ...
  const isMegaEvolution = useCallback((name) => {
    return name.includes('-mega');
  }, []);

  // Helper to check if a form should be displayed
  const isValidForm = useCallback((name) => {
  ...
  const formatFormName = useCallback((fullName, baseName) => {
  ...
  useEffect(() => {
    ...
    const loadEvolution = async () => {
      ...
      try {
        ...
        const parseEvolutionNode = async (node, parentId = null, level = 0) => {
          if (!node) return null;
          ...
          try {
            ...
            if (speciesDetail.varieties && speciesDetail.varieties.length > 1) {
              ...
              forms = forms.filter(f => f && isValidForm(f.name));
            }
          } catch (e) {
...
function EvolutionTreeDisplay({ node, currentPokemonId, isShiny, parentPosition = null }) {
  if (!node) return null;
...
function EeveeEvolutionDisplay({ evolutionData, currentPokemonId, isShiny, setIsShiny }) {
