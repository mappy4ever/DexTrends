...
interface SaleListing extends BaseListing {
  type: 'sale';
  price: number;
  negotiable: boolean;
}

interface TradeListing extends BaseListing {
  type: 'trade';
  wantedCards: string[];
  tradeCash?: number;
}

interface AuctionListing extends BaseListing {
  type: 'auction';
  currentBid: number;
  buyItNow?: number;
  bidders: number;
...
type Listing = SaleListing | TradeListing | AuctionListing;
...
interface TradingMarketplaceProps {
  userId?: string;
  onTradeRequest?: (listing: Listing) => void;
  onPurchase?: (listing: Listing, price?: number) => void;
}

const TradingMarketplace: React.FC<TradingMarketplaceProps> = ({ userId, onTradeRequest, onPurchase }) => {
  const [activeTab, setActiveTab] = useState<'marketplace' | 'my-trades' | 'watchlist' | 'sell'>('marketplace');
  ...
  const [sortBy, setSortBy] = useState<'newest' | 'price-low' | 'price-high' | 'ending-soon' | 'most-watched'>('newest');
  ...
  const handleTradeRequest = (listing: Listing) => {
      ...
      <div className="flex space-x-4">
        ...
        <div className="flex-1">
          ...
          {listing.type === 'sale' && (
            <div className="flex items-center justify-between">
              <div>
                ...
                {listing.negotiable && (
                  <span className="text-sm text-stone-500 dark:text-stone-300 ml-2">
                    (negotiable)
                  </span>
                )}
              </div>
          ...
          {listing.type === 'trade' && (
            <div className="flex items-center justify-between">
              <div>
                <span className="text-sm text-stone-600 dark:text-stone-300">Wants: </span>
