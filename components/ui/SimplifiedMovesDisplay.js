...
function SimplifiedMovesDisplay({ moves, pokemonName }) {
  ...
  const extractMachineNumber = (moveName, versionDetail) => {
  ...
  const renderMovesTable = (movesList, showLevel = false, showMachine = false) => {
  ...
  const tabs = [
    { id: 'level', label: 'Level Up', count: finalGroupedMoves['level-up'].length },
    { id: 'machine', label: 'TM/HM', count: finalGroupedMoves['machine'].length },
    { id: 'egg', label: 'Egg Moves', count: finalGroupedMoves['egg'].length },
    { id: 'tutor', label: 'Move Tutor', count: finalGroupedMoves['tutor'].length }
  ];
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4">
        {activeTab === 'level' && renderMovesTable(finalGroupedMoves['level-up'], true, false)}
