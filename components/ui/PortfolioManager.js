 ...
 */
const PortfolioManager = ({ userCards = [], onUpdateCard }) => {
  ...
  const [newPortfolio, setNewPortfolio] = useState({ name: '', description: '', type: 'collection' });
  ...
  const portfolioMetrics = useMemo(() => {
    ...
    const currentValue = cards.reduce((sum, card) => {
      const currentPrice = parseFloat(card.currentPrice || card.marketPrice || card.purchasePrice);
    ...
    const performanceByRarity = cards.reduce((acc, card) => {
      const rarity = card.rarity || 'Unknown';
      const cost = card.purchasePrice * card.quantity;
      const current = (parseFloat(card.currentPrice || card.marketPrice || card.purchasePrice)) * card.quantity;
  ...
  const generateMonthlyData = (cost, current) => {
  ...
  const addCardToPortfolio = () => {
    ...
    const portfolioCard = {
      ...
      quantity: parseInt(cardEntry.quantity),
  ...
  const removeCardFromPortfolio = (cardId) => {
    const updatedPortfolio = {
      ...selectedPortfolio,
  ...
  const tabs = [
    ...
    { id: 'performance', name: 'Performance', icon: <TrendingUpIcon className="h-5 w-5" /> },
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'holdings' && (
        <div className="space-y-4">
          ...
          {selectedPortfolio?.cards.length > 0 ? (
