...
interface TestData {
  tcg: TCGTestCard;
  pocket: PocketTestCard;
  pokedex: PokedexTestCard;
}
...
const QATestingTool: React.FC = () => {
  const [testMode, setTestMode] = useState<'all' | 'tcg' | 'pocket' | 'pokedex'>('all');
  ...
  const runVisualTest = (configName: string) => {
  ...
  const handleSelectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setTestMode(e.target.value as 'all' | 'tcg' | 'pocket' | 'pokedex');
  };

  const handleFailTest = (configName: string) => {
    setTestResults(prev => ({
      ...prev,
      [configName]: {
        status: 'failed',
            ...
            <div key={index} className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => handleFailTest(config.name)}
