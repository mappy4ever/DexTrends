...
const AdvancedKeyboardShortcuts = ({ 
  ...
  const [activeMode, setActiveMode] = useState('command'); // command, search, macro
  ...
  const saveShortcuts = (shortcuts, recent) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      const command = Object.values(advancedCommands).find(cmd => cmd.shortcut === shortcutKey);
      if (command) {
        e.preventDefault();
