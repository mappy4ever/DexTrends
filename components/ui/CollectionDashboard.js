...
const CollectionDashboard = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        ...
        const stats = calculateCollectionStats(favorites.cards || [], favorites.pokemon || []);
  ...
  const calculateCollectionStats = (cards, pokemon) => {
    const stats = {
  ...
  if (!collectionStats || (collectionStats.totalCards === 0 && collectionStats.totalPokemon === 0)) {
    return (
      ...
      {collectionStats.mostValuableCard && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center space-x-4">
            <img 
              src={collectionStats.mostValuableCard.images?.small}
              alt={collectionStats.mostValuableCard.name}
              className="w-20 h-28 object-cover rounded-lg shadow-md"
            />
            <div>
              <h4 className="text-xl font-bold text-gray-900 dark:text-white">{collectionStats.mostValuableCard.name}</h4>
              <p className="text-gray-600 dark:text-gray-400">${Math.round(getMockCardPrice(collectionStats.mostValuableCard))}</p>
              <p className="text-sm text-gray-500 dark:text-gray-500">{collectionStats.mostValuableCard.rarity}</p>
            </div>
          </div>
        </div>
      )}
