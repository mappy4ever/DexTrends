...
export default function TournamentSystem({ userId, onTournamentJoin, onCreateTournament }) {
  ...
  const generateBracket = (tournament) => {
    ...
    const players = Array.from({ length: Math.min(tournament.currentPlayers, 8) }, (_, i) => ({
      id: `player-${i}`,
      name: `Player ${i + 1}`,
      seed: i + 1,
  ...
  return (
    <div className="tournament-system space-y-6">
      ...
      <div className="flex space-x-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
        {[
          { id: 'tournaments', label: 'Active Tournaments', icon: <FaTrophy /> },
          { id: 'my-tournaments', label: 'My History', icon: <FaMedal /> },
          { id: 'meta-analysis', label: 'Meta Analysis', icon: <FaChartBar /> },
          { id: 'leaderboards', label: 'Leaderboards', icon: <FaCrown /> }
        ].map(tab => (
          <button
            key={tab.id}
