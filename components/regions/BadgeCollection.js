                  ...
                  <div className={`relative w-24 h-24 mx-auto mb-3 rounded-full ${
                    ...
                    <Image
                      ...
                      onError={(e) => {
                        ...
                        const currentPattern = patterns.findIndex(p => e.target.src.includes(p));
                        if (currentPattern < patterns.length - 1) {
                          e.target.src = patterns[currentPattern + 1];
                        } else {
                          e.target.style.display = 'none';
                          e.target.parentElement.innerHTML = `<div class="badge-fallback">${index + 1}</div>`;
                        }
                      }}
                    />
                  </div>
