...
interface ExtendedProfessor extends Professor {
  secondary?: Professor;
}
...
interface StarterShowcaseEnhancedProps {
  region: Region;
  theme: string;
}

const StarterShowcaseEnhanced: React.FC<StarterShowcaseEnhancedProps> = ({ region, theme }) => {
  const [selectedStarter, setSelectedStarter] = useState(0);
  const [showEvolutions, setShowEvolutions] = useState(false);

  // Professor data
  const professorData: Record<string, ExtendedProfessor> = {
  ...
  const starterEvolutions: Record<number, Evolution[]> = {
    // Kanto
  ...
  const starterInfo: Record<string, StarterInfo> = {
    'Bulbasaur': { 
  ...
  const getEvolutionTypes = (pokemonId: number, baseTypes: string[]): string[] => {
    // Type data for evolutions
    const evolutionTypeData: Record<number, string[]> = {
      // Kanto
