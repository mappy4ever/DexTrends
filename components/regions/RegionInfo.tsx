...
const GlassOrb: React.FC<{
  ...
  return (
    ...
    >
      ...
      {/* Main glass orb */}
      <div className="relative w-full h-full rounded-full backdrop-blur-xl bg-white/95 dark:bg-stone-800/95 border border-white/60 dark:border-stone-700/60 shadow-2xl overflow-hidden">
        {/* Inner gradient */}
        ...
        <div className="relative h-full flex flex-col items-center justify-center p-4">
          ...
          <div className="text-xs font-bold text-stone-800 dark:text-stone-300 uppercase tracking-wider mb-1 drop-shadow-sm">
            {label}
          </div>
          <div className="text-xl font-bold text-stone-900 dark:text-white text-center drop-shadow-sm">
            {value}
          </div>
        </div>
...
const RegionInfo: React.FC<RegionInfoProps> = ({ region, theme }) => {
  ...
  const generationColors: Record<number, string> = {
    1: 'from-red-400 to-amber-400',
    2: 'from-yellow-400 to-orange-400',
    3: 'from-emerald-400 to-teal-400',
    4: 'from-amber-400 to-amber-400',
    5: 'from-stone-500 to-stone-600',
    6: 'from-pink-400 to-rose-400',
    7: 'from-orange-400 to-amber-400',
    8: 'from-amber-400 to-violet-400',
    9: 'from-violet-400 to-fuchsia-400'
  };

  const genColor = generationColors[Number(region.generation)] || 'from-stone-400 to-stone-500';
    ...
    <div className="relative py-16 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <h3 className="text-2xl font-bold text-center mb-8 text-stone-800 dark:text-white">
            Available Games
          </h3>
          ...
          >
            ...
            <div className="flex justify-center gap-8">
              {region.starterIds.map((id, index) => (
                ...
                >
                  <div className="w-32 h-32 rounded-full backdrop-blur-xl bg-white/80 dark:bg-stone-800/80 border border-white/30 dark:border-stone-700/30 shadow-xl overflow-hidden">
                    <div className={`absolute inset-0 bg-gradient-to-br ${
                      index === 0 ? 'from-green-400/20 to-emerald-400/20' :
                      index === 1 ? 'from-red-400/20 to-orange-400/20' :
                      'from-amber-400/20 to-cyan-400/20'
                    }`} />
                    <img
                  ...
                  <p className="text-center mt-2 font-semibold text-stone-700 dark:text-stone-300">
