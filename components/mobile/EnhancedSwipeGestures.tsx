...
interface EnhancedSwipeGesturesProps {
  ...
  onPinchZoom?: (data: PinchData) => void;
...
const EnhancedSwipeGestures: React.FC<EnhancedSwipeGesturesProps> = ({ 
  children, 
  onSwipeLeft, 
  onSwipeRight, 
  onSwipeUp, 
  ...
  const calculateVelocity = useCallback((deltaX: number, deltaY: number, deltaTime: number): Velocity => {
  ...
  const updateSwipeIndicator = useCallback((direction: SwipeState['direction'], progress: number) => {
