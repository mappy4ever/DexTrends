...
const FloatingActionButton = ({
  ...
  const handleMainClick = useCallback(() => {
    ...
    if (actions.length > 0) {
      ...
      logger.debug('FAB expanded:', !isExpanded);
    } else if (onClick) {
      onClick();
  ...
  const handleActionClick = useCallback((action, index) => {
    ...
    logger.debug('FAB action clicked:', action.label);
  }, [utils]);
...
export const QuickActionFAB = ({ onSearch, onScan, onFavorites, onShare, ...props }) => {
  const actions = [
    {
      icon: '🔍',
      label: 'Search',
...
export const CollectionFAB = ({ onAdd, onCreate, onImport, onExport, ...props }) => {
  const actions = [
    {
      icon: '➕',
      label: 'Add Card',
