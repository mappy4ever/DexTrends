...
type ShareMethod = 'copy' | 'twitter' | 'facebook' | 'whatsapp' | 'telegram' | 'email' | 'reddit' | 'native';
...
interface MobileShareProps {
  ...
  image?: string;
  cardData?: CardData;
  onShareComplete?: (method: ShareMethod, data: ShareData) => void;
...
// Check if share is available on navigator
...
const MobileShare: React.FC<MobileShareProps> = ({
  ...
  const [isSupported] = useState(() => {
    ...
    return typeof navigator !== 'undefined' && !!nav.share;
  });
  ...
  const formatShareData = useCallback((): ShareData => {
    if (cardData) {
  ...
  const shareFallback = useCallback(async (method: ShareMethod): Promise<boolean> => {
    const shareData = formatShareData();
    setIsSharing(true);
