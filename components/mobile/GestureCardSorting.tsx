...
interface GestureCardSortingProps {
  ...
  className?: string;
  enableGestureSorting?: boolean;
  enableGestureFiltering?: boolean;
  swipeActions?: SwipeActions;
}

interface SortGesture {
  sort: string;
  label: string;
}
...
const defaultSwipeActions: SwipeActions = {
...
const GestureCardSorting: React.FC<GestureCardSortingProps> = ({
  cards = [],
  onCardAction,
  onSort,
  onFilter,
  ...
  const [swipeDirection, setSwipeDirection] = useState<keyof SwipeActions | null>(null);
  ...
  const handleGesturePattern = useCallback((pattern: string) => {
  ...
  const handleDoubleTap = useCallback((position: Position) => {
    ...
    if (tappedCard) {
      ...
      if (cardId) {
        handleCardSwipe(cardId, 'down', { type: 'double-tap' });
      }
    }
  ...
  const getFilteredCards = (): Card[] => {
    ...
    switch (filterMode) {
      ...
      case 'collection':
        ...
        break;
      case 'high-value':
        filtered = filtered.filter(card => (card.price || 0) > 100);
        break;
      default:
        // Show all cards
        break;
