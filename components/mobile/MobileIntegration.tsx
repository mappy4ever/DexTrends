...
interface AdaptiveLoadingEvent extends Event {
  detail: {
    strategy: string;
  };
}

interface BatteryOptimizationEvent extends Event {
  detail: {
    level: string;
  };
}

const MobileIntegration: React.FC<MobileIntegrationProps> = ({ 
  children, 
  ...
  useEffect(() => {
    ...
    window.addEventListener('deepLinkReceived', handleDeepLink as EventListener);
    
    // Setup share target handling
    const handleShareTarget = (event: Event) => {
    ...
    const handleLoadingStrategyChange = (event: AdaptiveLoadingEvent) => {
      const { detail } = event;
    ...
    window.addEventListener('adaptiveLoadingStrategyChange', handleLoadingStrategyChange as EventListener);

    // Setup battery optimization updates
    const handleBatteryOptimization = (event: BatteryOptimizationEvent) => {
      const { detail } = event;
      setMobileFeatures(prev => ({
        ...prev,
    ...
    window.addEventListener('batteryOptimizationChange', handleBatteryOptimization as EventListener);
    ...
    return () => {
      window.removeEventListener('deepLinkReceived', handleDeepLink as EventListener);
      window.removeEventListener('shareTargetReceived', handleShareTarget);
      window.removeEventListener('adaptiveLoadingStrategyChange', handleLoadingStrategyChange as EventListener);
      window.removeEventListener('batteryOptimizationChange', handleBatteryOptimization as EventListener);
  ...
  return (
    <div className="mobile-integration">
        ...
        >
          <div className="p-4">
            <VoiceSearch
              ...
              onError={() => showNotification('Voice search error', 'error')}
      ...
      {enableCardScanner && (
        ...
        >
          <CardScanner
            ...
            onError={() => showNotification('Scanner error', 'error')}
            autoCapture={true}
            overlayEnabled={true}
          />
        </BottomSheet>
      ...
      {enablePushNotifications && (
        <PushNotifications
          onPermissionChange={(permission: NotificationPermission) => {
