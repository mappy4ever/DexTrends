...
const PushNotifications = ({
  ...
  const initializePushSubscription = useCallback(async () => {
    try {
  ...
  // Add price alert
  const addPriceAlert = useCallback((cardId, cardName, targetPrice, condition = 'below') => {
    const alert = {
      id: `price-${Date.now()}`,
      type: 'price',
      cardId,
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      if (event.data && event.data.type === 'push-notification') {
        ...
        logger.debug('Push notification received:', event.data);
      }
    };
