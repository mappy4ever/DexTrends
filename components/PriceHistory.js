...
// Simple line chart component (you can replace with a proper charting library later)
function SimpleLineChart({ data, title, height = 200 }) {
  ...
  const maxPrice = Math.max(...data.map(d => parseFloat(d.price_market) || 0));
  const minPrice = Math.min(...data.map(d => parseFloat(d.price_market) || 0));
          ...
          <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
            ...
            <polyline
              ...
              points={data.map((point, index) => {
                ...
                const y = 100 - (((parseFloat(point.price_market) || 0) - minPrice) / priceRange) * 100;
                return `${x},${y}`;
              }).join(' ')}
            />
...
function PriceStats({ stats, loading }) {
  if (loading) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-lg">
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-lg">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-lg font-bold text-gray-900 dark:text-white">
            ${parseFloat(stats.current_price || 0).toFixed(2)}
          </p>
        </div>
      ...
      {stats.price_volatility && (
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
          ...
          <p className="text-sm text-gray-900 dark:text-white">
            ${parseFloat(stats.price_volatility).toFixed(2)}
          </p>
        </div>
      )}
    </div>
...
// Main price history component
export default function PriceHistory({ cardId, cardName, variantType = 'holofoil' }) {
                ...
                {priceHistory.slice(0, 10).map((row, index) => (
                  <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900 dark:text-white font-medium">
                      ${parseFloat(row.price_market || 0).toFixed(2)}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                      ${parseFloat(row.price_low || 0).toFixed(2)}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                      ${parseFloat(row.price_high || 0).toFixed(2)}
                    </td>
                    <td className="px-4 py-3 text-sm">
                      {row.day_change_percent && (
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                          parseFloat(row.day_change_percent) > 0 
                            ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100'
                            : parseFloat(row.day_change_percent) < 0
                            ? 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100'
                            : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'
                        }`}>
                          {parseFloat(row.day_change_percent) > 0 ? '+' : ''}{parseFloat(row.day_change_percent).toFixed(1)}%
                        </span>
                      )}
                    </td>
                  </tr>
