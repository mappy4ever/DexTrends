...
const PokemonStatsTab: React.FC<PokemonStatsTabProps> = ({ pokemonDetails }) => {
          ...
          <div className="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
            ...
            <div className="text-sm text-green-700 dark:text-green-300">
              ...
              <div className="mt-2 text-xs">
                {(pokemonDetails.stats.find(s => s.stat.name === 'speed')?.base_stat || 0) >= 100 
                  ? 'Very Fast' 
                  : (pokemonDetails.stats.find(s => s.stat.name === 'speed')?.base_stat || 0) >= 80 
                  ? 'Fast' 
                  : (pokemonDetails.stats.find(s => s.stat.name === 'speed')?.base_stat || 0) >= 60 
                  ? 'Average' 
                  : (pokemonDetails.stats.find(s => s.stat.name === 'speed')?.base_stat || 0) >= 40 
                  ? 'Slow' 
                  : 'Very Slow'}
              </div>
            </div>
