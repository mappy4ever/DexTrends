...
const MetricCard: React.FC<MetricCardProps> = ({ 
  ...
  const getTrendIcon = (trend: 'up' | 'down' | null): string => {
...
const CoreWebVitals: React.FC<CoreWebVitalsProps> = ({ vitals }) => {
  const getVitalStatus = (name: keyof Vitals, value?: number): 'good' | 'needs-improvement' | 'poor' | 'unknown' => {
    const thresholds = {
  ...
  const vitalDisplayNames: Record<keyof Vitals, string> = {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {(Object.entries(vitalDisplayNames) as Array<[keyof Vitals, string]>).map(([key, name]) => {
...
const APIPerformance: React.FC<APIPerformanceProps> = ({ apiMetrics }) => {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
...
const ComponentPerformance: React.FC<ComponentPerformanceProps> = ({ performanceData }) => {
  ...
  useEffect(() => {
    ...
    Object.entries(metrics).forEach(([key, values]) => {
      if (key.includes('component-')) {
        ...
        componentData[metricType].push(...(values as Metric[]));
      }
    });
...
const MemoryUsage: React.FC<MemoryUsageProps> = ({ performanceData }) => {
...
const OptimizationSuggestions: React.FC<OptimizationSuggestionsProps> = ({ performanceData }) => {
  ...
  useEffect(() => {
    const generateSuggestions = () => {
      ...
      const slowRenders = renderTimes.filter((m: Metric) => m.value > 16);
...
const PerformanceDashboard: React.FC<PerformanceDashboardProps> = ({ isOpen = false, onClose = () => {} }) => {
