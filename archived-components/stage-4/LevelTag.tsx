 ...
 */
export default function LevelTag({ level, size = 'md', className = '' }: LevelTagProps) {
...
interface CompactLevelTagProps {
  level: number;
  className?: string;
}
 ...
 */
export function CompactLevelTag({ level, className = "" }: CompactLevelTagProps) {
  if (!level || level === 0) return null;
