...
import { motion, useInView, useMotionValue, useSpring, MotionValue, SpringOptions } from 'framer-motion';
...
interface GymLeaderShowcaseProps {
  leader: GymLeader;
  index: number;
  isExpanded?: boolean;
  onToggle?: () => void;
...
type PokemonType = 
  ...
  | 'bug' | 'rock' | 'ghost' | 'steel' | 'fairy' | 'normal';

const GymLeaderShowcase: React.FC<GymLeaderShowcaseProps> = ({ leader, index, isExpanded, onToggle }) => {
  ...
  const springConfig: SpringOptions = { stiffness: 300, damping: 30 };
  ...
  const getTypeGradient = (type: string): string => {
    const gradients: Record<PokemonType, string> = {
    ...
    return gradients[type as PokemonType] || gradients.normal;
  };
            ...
            {leader.image ? (
              <Image
                ...
                onError={(e) => {
                  ...
                  target.src = '/images/placeholder-gym-leader.png';
                }}
              />
            ) : (
